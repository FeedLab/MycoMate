...
public static class IngredientEndpoints
{
    public static IEndpointRouteBuilder MapIngredientEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", async (Guid projectId, ClaimsPrincipal user, IProjectRepository projectRepo, IIngredientRepository ingredientRepo, CancellationToken ct) =>
        {
            var userId = user.FindFirstValue(ClaimTypes.NameIdentifier)!;
            var project = await projectRepo.GetByIdAsync(projectId, ct);
            if (project is null || await projectRepo.GetUserRoleAsync(projectId, userId, ct) is null)
                return Results.NotFound();

            var ingredients = await ingredientRepo.GetVisibleAsync(project.OwnerId, ct);
            var response = ingredients.Select(i => new IngredientResponse(i.Id, i.ShortName, i.DisplayName, i.Information, i.MoistureContent, i.Created));
            return Results.Ok(response);
        })
        .WithName("GetIngredients");

        group.MapPost("/", async (Guid projectId, CreateIngredientRequest req, ClaimsPrincipal user, IProjectRepository projectRepo, IIngredientRepository ingredientRepo, CancellationToken ct) =>
        {
            var userId = user.FindFirstValue(ClaimTypes.NameIdentifier)!;
            ...
            var response = new IngredientResponse(ingredient.Id, ingredient.ShortName, ingredient.DisplayName, ingredient.Information, ingredient.MoistureContent, ingredient.Created);
